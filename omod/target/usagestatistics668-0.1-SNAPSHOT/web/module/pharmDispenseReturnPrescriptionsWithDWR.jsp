...
<!--  SINCE WE WANT TO AVOID RE-LOADING THE PAGE WE WOULD USE DWR; HOWEVER, WITH THIS EXAMPLE
      WE WILL ONLY SIMULATE THE USE OF DWR - THE OBJECTIVE IS TO CREATE SEVERAL PAGE OPTIONS:
      1. JUST FIND A PATIENT
      2. DISPENSE PRESCRIPTION
      3. RETURN PRESCRIPTION

      WE ARE CREATING ALL DIVS IN ADVANCE, BUT KEEPING APPROPRIATE DIVS HIDDEN UNTIL NEEDED
-->

<h2><spring:message code="usagestatistics668.dispenseprescript" /></h2>
...
<script type="text/javascript">
	...
	$j(document).ready(function(){
		...
		$j('#returnPres').click(function(){
                    $j('#patientInfoReturn').attr("hidden",false); 
                    $j('#patientInfoDispense').attr("hidden",true); 
                    $j('#dispensePres').attr("hidden",false); 
                    $j('#findPatient').attr("hidden",true); 
                    $j('#pName1').html(selectedPatient.givenName  + " " + selectedPatient.familyName)
                    $j('#pAge1').html(selectedPatient.age)
                    $j('#pId1').attr("value",selectedPatient.patientId);               
                    $j('#returnPres').attr("hidden",true); 
	   		});
		$j('#dispensePres').click(function(){
                    setPatientInfoForDispense(selectedPatient.patientId) 
                    $j('#patientInfoReturn').attr("hidden",true); 
                    $j('#returnPres').attr("hidden",false);
	   		});
 	});
	...
	function selectPatient(input){
            ...
            setPatientInfoForDispense(input)
          }
        
        function setPatientInfoForDispense(patient_id) {
            $j('#pName').html(selectedPatient.givenName  + " " + selectedPatient.familyName)
            $j('#pAge').html(selectedPatient.age)
            $j('#pId').attr("value",patient_id);
            $j('#patientInfoDispense').attr("hidden",false); 
            $j('#pharmSelector').attr("hidden",false); 
            $j('#findPatient').attr("hidden",true);         
            $j('#dispensePres').attr("hidden",true);
        }
...
<a href="/openmrs/admin">Administration</a>
 ...
 -->
			...
			<div id="findPatient">
				...
				<b class="boxHeader" style="padding-left: 15px; padding-right: 15px;"><spring:message code="usagestatistics668.findpatient" /></b>
				<div class="box" style="padding: 15px 15px 15px 15px;">
					
                                    <b><spring:message code="usagestatistics668.findpatient" /></b>
              ...
              <div> 
                                    ...
                                    <div id="pharmSelector" hidden="true" style="float:left">
                                        <a href="pharmDispenseReturnPrescriptionsWithDWR.htm"><spring:message code="Select Another Patient" /></a>
                                          <br>  
                                          <input type="button" id="dispensePres" value="Dispense Prescription"/> 
                                          <br>
                                        <input type="button" id="returnPres" value="Return Prescription"/>   
                                    </div>
 <div id="patientInfoDispense" style="float:left" hidden="true">
	...
	<input type="button" value="Submit Prescription"/>
 </div>
                                          
<div id="patientInfoReturn" style="float:left" hidden="true">
     <h2>RETURN PRESCRIPTION</h2>
     <table>
         <thead>Patient Info</thead>
         <tr><td id="pName1""></td></tr>
         <tr><td id="pAge1"></td></tr>
     </table>
	<input id="pId1"type="hidden" name="patient_id" />
	Drug: <input type="text" name="drug"/><br/>
	<input type="button" value="Return Prescription"/>
 </div>                                          
